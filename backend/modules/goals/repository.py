...
class GoalRepository:
    ...
    async def get_by_id_with_subgoals(self, goal_id: UUID) -> Optional[Goal]:
        ...
        result = await self.db.execute(
            select(Goal).options(joinedload(Goal.subgoals)).filter(Goal.id == goal_id)
        )
        return result.scalars().first()
    ...
    async def get_by_project(self, project_id: UUID, skip: int = 0, limit: int = 100) -> Sequence[Goal]:
        ...
        result = await self.db.execute(
            select(Goal).filter(Goal.project_id == project_id).offset(skip).limit(limit)
        )
    ...
    async def get_subgoals(self, parent_goal_id: UUID) -> Sequence[Goal]:
        ...
        result = await self.db.execute(
            select(Goal).filter(Goal.parent_goal_id == parent_goal_id)
        )
        return result.scalars().all()
    ...
    async def count_by_project(self, project_id: UUID) -> int:
        ...
        result = await self.db.execute(
            select(func.count()).filter(Goal.project_id == project_id)
        )
    ...
    async def get_by_status(self, project_id: UUID, status: str) -> Sequence[Goal]:
        """Get goals by status."""
        result = await self.db.execute(
            select(Goal).filter(and_(Goal.project_id == project_id, Goal.status == status))
        )
        return result.scalars().all()
