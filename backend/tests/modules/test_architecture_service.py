...
class TestArchitectureService:
    ...
    async def test_create_module_with_parent(self, async_session: AsyncSession, sample_project: Project) -> None:
        ...
        # Create parent
        parent_data = ArchitectureModuleCreate(project_id=sample_project.id, name="Backend", module_type="package")
        parent = await service.create_module(parent_data)

        # Create child
        child_data = ArchitectureModuleCreate(
            project_id=sample_project.id, parent_id=parent.id, name="UserService", module_type="service"
        )
    ...
    async def test_create_dependency_prevents_self_reference(
        ...
        module_data = ArchitectureModuleCreate(project_id=sample_project.id, name="Service", module_type="service")
    ...
    async def test_delete_module_checks_impact(self, async_session: AsyncSession, sample_project: Project) -> None:
        ...
        # Create module
        data = ArchitectureModuleCreate(project_id=sample_project.id, name="TestModule", module_type="module")
        module = await service.create_module(data)
