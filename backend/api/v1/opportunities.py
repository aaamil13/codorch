...
async def create_opportunity(
    ...
    try:
        opportunity = await service.create_opportunity(project_id, opportunity_data)
...
async def list_opportunities(
    ...
    service = OpportunityService(db)
    opportunities = await service.list_opportunities(project_id, skip, limit)
...
async def get_opportunity(
    ...
    # Verify ownership
    project = (await db.execute(select(Project).filter(Project.id == opportunity.project_id))).scalars().first()
...
async def update_opportunity(
    ...
    try:
        updated = await service.update_opportunity(opportunity_id, opportunity_update)
...
async def get_top_opportunities(
    ...
    service = OpportunityService(db)
    opportunities = await service.get_top_opportunities(project_id, limit)
    return [OpportunityResponse.model_validate(opp) for opp in opportunities]
...
async def compare_opportunities(
    ...
    for opp_id in request.opportunity_ids:
        opportunity = await service.get_opportunity(opp_id)
        if not opportunity:
            raise HTTPException(
    ...
    try:
        comparison = await service.compare_opportunities(request.opportunity_ids)
        return comparison
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
